...
//         释放资源
    ...
    private static void write(Socket client, ClientInfo clientInfo) throws IOException {
        ...
        while (true) {
            if (!clientInfo.getOtherPartyNames().isEmpty()) {
                System.out.println("You can send files to this / these client(s), please choose one:");
            ...
            try {
                ...
                do {
                    try {
                        ...
                        if (jsonObject == null) {
                            continue;
                        } else if (jsonObject.containsKey("eccPublicKeyStr")) {
                            System.out.println("[system]: receive public message ==> " + jsonObject.toString());
                            clientInfo.setOtherPartyNames(jsonObject.getString("name"));
                            clientInfo.setOtherPartyEccPublicKeyStrs(jsonObject.getString("name"), jsonObject.getString("eccPublicKeyStr"));
